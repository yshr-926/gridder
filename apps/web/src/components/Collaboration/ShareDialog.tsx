...
export const ShareDialog = ({ isOpen, onClose }: ShareDialogProps) => {
  ...
  useEffect(() => {
    if (room?.id && isOpen) {
      ...
      apiClient
        .get<HasPassphraseResponse>(`/api/rooms/${room.id}/has-passphrase`)
  ...
  const handleSetPassphrase = useCallback(async () => {
    ...
    try {
      const data = await apiClient.post<SetPassphraseResponse>(
        `/api/rooms/${room.id}/passphrase`,
        {
          passphrase: passphrase,
          currentPassphrase: hasPassphrase ? currentPassphrase : undefined,
        }
      );
  ...
  const handleClearPassphrase = useCallback(async () => {
    ...
    try {
      await apiClient.post<SetPassphraseResponse>(
        `/api/rooms/${room.id}/passphrase`,
        {
          passphrase: null,
